          ...
          <div class="mt-8">
            ${renderInput({
              ...
              action: `
                <button
                  type="button"
                  class="absolute top-1/2 right-3 -translate-y-1/2 xl:hidden"
                  data-add-expense
                  aria-label="Добавить трату"
                >
                  <img src="${enterIcon}" width="24" height="24" alt="" class="h-6 w-6 object-contain" />
                </button>
              `,
            })}
          </div>
...
export function initMainPage(
  ...
  const addExpenseButton = root.querySelector('[data-add-expense]');
  ...
  if (addExpenseButton instanceof HTMLButtonElement) {
    addExpenseButton.addEventListener('click', submitExpense);
  }
